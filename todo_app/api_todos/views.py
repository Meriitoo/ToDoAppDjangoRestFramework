...
class ListCreateTodoApiView(rest_generic_views.ListCreateAPIView):
    ...
    # if we create to do we have to log in
    permission_classes = (
    ...
    # get all to do for current user
...
class DetailsTodoApiView(rest_generic_views.RetrieveUpdateAPIView):
    ...
    # when user is not in login, and want to access the url, user can't
    def get_object(self):
...
class ListCategoriesApiView(rest_generic_views.ListCreateAPIView):
    ...
    # filter to do all, if it has something, if it's empty, we can't see
    def get_queryset(self):
